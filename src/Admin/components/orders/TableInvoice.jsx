...
const TableInvoice = ({ order }) => {
    ...
    if (!order.item) return <h1>Đang xử lý</h1>;
    return (
        <>
            <Table>
                <Thead>
                    <TrHead>
                        ...
                        <TdHead>Tên sản phẩm</TdHead>
                        <TdHead textAlign='center'>Số lượng</TdHead>
                        <TdHead textAlign='center'>Đơn giá</TdHead>
                        <TdHead textAlign='center'>Thành tiền</TdHead>
                    </TrHead>
                </Thead>
                <Tbody>
                    {order.item.map((oneProduct, index) => {
                        return (
                            <Tr key={oneProduct.product_id}>
                                ...
                                <Td textAlign='center'>{oneProduct.price}đ</Td>
                                <Td textAlign='center' color='red'>
                                    {oneProduct.amount * oneProduct.price}đ
                                </Td>
                            </Tr>
                        );
                    })}
